...
public class CafeDao {
    ...
    /*
        static 메소드는 생성자를 호출하지 않고 클래스명으로 바로 호출을 하기 때문에 메소드 호출 전에 무언가 준비작업을 하고 싶다면
        static 블럭 안에서 하면 된다.
        static 블럭은 해당 클래스를 최초로 사용할때 한번만 실행되기 때문에 초기화 작업을 하기에 적당한 블럭이다
     */
    static {
        // 1. 객체를 생성해서 static 필드에 저장해두면
        dao = new CafeDao();
    }
    // 외부에서 객체생성하지 못하도록
    private CafeDao() { }
